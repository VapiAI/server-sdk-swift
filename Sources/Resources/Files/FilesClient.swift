...
public final class FilesClient: Sendable {
    ...
    ///
    /// - Parameter purpose: Only return files with this purpose. When omitted, files of every purpose except composer attachments are returned.
    /// - Parameter requestOptions: Additional options for configuring the request, such as custom headers or timeout settings.
    public func list(purpose: ListFilesRequestPurpose? = nil, requestOptions: RequestOptions? = nil) async throws -> [File] {
        return try await httpClient.performRequest(
            method: .get,
            path: "/file",
            queryParams: [
                "purpose": purpose.map { .string($0.rawValue) }
            ],
