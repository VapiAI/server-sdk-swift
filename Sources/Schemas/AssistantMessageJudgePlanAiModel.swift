...
public enum AssistantMessageJudgePlanAiModel: Codable, Hashable, Sendable {
    case anthropic(EvalAnthropicModel)
    case customLlm(EvalCustomModel)
    case google(EvalGoogleModel)
    case openai(EvalOpenAiModel)
    ...
    public init(from decoder: Decoder) throws {
        ...
        switch discriminant {
        case "anthropic":
            self = .anthropic(try EvalAnthropicModel(from: decoder))
        case "custom-llm":
            self = .customLlm(try EvalCustomModel(from: decoder))
        case "google":
            self = .google(try EvalGoogleModel(from: decoder))
        case "openai":
            self = .openai(try EvalOpenAiModel(from: decoder))
        default:
